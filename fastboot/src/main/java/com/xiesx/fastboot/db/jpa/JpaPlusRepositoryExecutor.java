...
public class JpaPlusRepositoryExecutor<T, ID> extends SimpleJpaRepository<T, ID>
  ...
  @Transactional
  @Override
  public int update(Path<T> key, T val, Predicate... predicate) {
    return (int) jpaQueryFactory.update(path).set(key, val).where(predicate).execute();
  }

  @Transactional
  @Override
  public <V> int update(Path<V> key, Expression<? extends V> expression, Predicate... predicate) {
    int rows = (int) jpaQueryFactory.update(path).set(key, expression).where(predicate).execute();
