...
public class FastJsonCfg implements WebMvcConfigurer {
  ...
  public FastJsonConfig newFastJsonConfig() {
    ...
    config.setDateFormat(
        ObjectUtil.defaultIfBlank(config.getDateFormat(), DatePattern.NORM_DATETIME_PATTERN));
    // 序列化 Writer ==============================
    // 特性
    config.setWriterFeatures(
        ArrayUtil.addAll(FastJsonProperties.SERIALIZER_FEATURES, config.getWriterFeatures()));
    // 注冊
    ...
    config.setReaderFeatures(
        ArrayUtil.addAll(FastJsonProperties.DESERIALIZER_FEATURES, config.getReaderFeatures()));
    // 注冊
