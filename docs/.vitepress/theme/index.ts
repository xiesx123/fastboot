...
// vercount
import { useVisitData } from "./utils/vercount";
...
export const Theme: ThemeConfig = {
  ...
  // 使用注入插槽的包装组件覆盖
  enhanceApp(ctx) {
    ctx.app.component("NavLinks", navlinks); // 导航页
    // 统计访问
    if (inBrowser) {
...
export default {
  ...
  enhanceApp(ctx) {
    ...
    ctx.app.use(
      NolebasePagePropertiesPlugin<{ progress: number }>() as Plugin,
      {
        properties: {
          zh: [
            {
  ...
  setup() {
    ...
    // 图片预览 -> https://fengyuanchen.github.io/viewerjs
